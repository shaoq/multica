...
On macOS, get the stable Bonjour name with `scutil --get LocalHostName`, append `.local`, and use it instead of the changing Wi-Fi address. For example, if the command prints `your-mac`, set:
...
Build Backend/Web from the current checkout and start the stack:

```bash
make selfhost-build
```

Connect the phone and Mac to the same trusted Wi-Fi, then open `http://your-mac.local:3000` in Safari. Browser HTTP and WebSocket traffic use the frontend's same-origin proxy, so both public URL overrides stay empty. A native iOS client can use `http://your-mac.local:8080` as its Backend base URL. PostgreSQL remains unpublished. `0.0.0.0` exposes the raw Web and Backend ports on every Mac interface, so keep the macOS firewall enabled, use this only on a trusted network, and do not configure router port forwarding.

### Remote access

Docker Compose binds `3000` and `8080` to `127.0.0.1` by default. The trusted-LAN setup above deliberately switches to `0.0.0.0`; do not expose those ports through the router or use that setup as a public deployment. For internet access, return to loopback binding and use a reverse proxy with HTTPS instead.
